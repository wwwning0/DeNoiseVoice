# dccrn.py (Final, Fully Corrected and Stabilized Version)
# (和上次提供给您的版本一致，这里再次提供以确保您使用的是正确的最终版本)
import torch
...
class DCCRN(nn.Module):
    def __init__(
        ...
        if self.use_clstm:
            ...
            for idx in range(rnn_layers):
                rnns.append(
                    NavieComplexLSTM(
                        input_size=freq_dim * self.kernel_num[-1] if idx == 0 else self.rnn_units,
                        hidden_size=self.rnn_units,
                        bidirectional=bidirectional,
                        batch_first=False,
                        projection_dim=freq_dim * self.kernel_num[-1] if idx == rnn_layers - 1 else None,
                    )
                )
            self.enhance = nn.Sequential(*rnns)
        else:
            self.enhance = nn.LSTM(
                input_size=freq_dim * self.kernel_num[-1],
                hidden_size=self.rnn_units,
                num_layers=2,
                dropout=0.0,
                bidirectional=bidirectional,
            ...
            self.tranform = nn.Linear(self.rnn_units * fac, freq_dim * self.kernel_num[-1])
    ...
    def forward(self, inputs, lens=None):
        ...
        spec_mags = (real ** 2 + imag ** 2 + eps) ** 0.5
        is_zero_mask_spec = (spec_mags < eps)
        real_stabilized_spec = real + is_zero_mask_spec.float() * eps
        spec_phase = torch.atan2(imag, real_stabilized_spec)
        ...
        if self.use_clstm:
            r_rnn_in, i_rnn_in = out[:, :, :channels // 2], out[:, :, channels // 2:]
        ...
        if self.masking_mode == 'E':
            ...
            is_zero_mask_mask = ((real_phase.abs() < eps) & (imag_phase.abs() < eps))
            real_phase_stabilized = real_phase + is_zero_mask_mask.float() * eps
            mask_phase = torch.atan2(imag_phase, real_phase_stabilized)
            ...
            real, imag = real * mask_real - imag * mask_imag, real * imag + imag * mask_real
